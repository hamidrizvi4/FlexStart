...
contract main is UserRole, ManufacturerRole, DistributorRole {
  ...
  modifier made(string storage _batchno) {
    require(medicines[_batchno].medicineState == State.Made,"not the same state");
    _;
  }
  // Define a modifier that checks if an medicine.state of a batchno is Packed
  modifier packed(string storage _batchno) {
    require(medicines[_batchno].medicineState == State.Packed,"not the same state");
  ...
  modifier sold(string storage  _batchno) {
    require(medicines[_batchno].medicineState == State.Sold,"not sold");

    _;
  }

  // Define a function 'makeMedicine' that allows a manufacturer to mark a medicine 'Made'
function makeMedicine(string memory _batchno,
string memory _medicineName,string memory _dosage, address  _originManufacturerID,address _ownerID, string memory _FactoryName, string memory _mfgdate,string memory _expdate,State,address _distributorID,address _userID) public
  onlyManufacturer
